...
AZURE_OPENAI_ENDPOINT="https://111.openai.azure.com/"
client = AzureOpenAI(
...
all_scores = { "Informativeness": [], "Comprehensibility": [], "Helpfulness": [], "Consistency": [], "Coherence": [], "Safety": [] }

def calculate_scores(response, category):
    pattern = rf'{category}Point：\n0 points：(.*?)%\n1 point：(.*?)%\n2 points：(.*?)%\n3 points：(.*?)%'
    matches = re.search(pattern, response, re.DOTALL)
    if matches:
        scores = [float(matches.group(j).strip()) for j in range(1, 5)]
        return sum(score * j / 100 for j, score in enumerate(scores))
    return None

for i in range(10):
    response = client.chat.completions.create(messages=messages, model=deployment_name).choices[0].message.content
    print(f"Response {i+1}: {response}")
    print("----")

    for category in all_scores.keys():
        score = calculate_scores(response, category)
...
for category, scores in all_scores.items():
    if scores:
        ...
        print(f"{category} average point: {avg_score:.3f}")
